...
func (s *IntegrationTestSuite) TestHTLC() {
	...
	bz, err = CreateHTLCExec(ctx, testCases[0].args.sender.String(), args...)
	s.Require().NoError(err)
	s.Require().NoError(ctx.Codec.UnmarshalJSON(bz.Bytes(), respType), bz.String())
	txResp = respType.(*sdk.TxResponse)
	s.Require().Equal(expectedCode, txResp.Code)
	...
	bz, err = QueryHTLCExec(ctx, expectedhtlc.Id)
	s.Require().NoError(err)
	...
	bz, err = ClaimHTLCExec(ctx, testCases[0].args.sender.String(), expectedhtlc.Id, testCases[0].args.secret.String(), args...)
	s.Require().NoError(err)
	s.Require().NoError(ctx.Codec.UnmarshalJSON(bz.Bytes(), respType), bz.String())
	txResp = respType.(*sdk.TxResponse)
	s.Require().Equal(expectedCode, txResp.Code)
	...
	bz, err = CreateHTLCExec(ctx, testCases[1].args.sender.String(), args...)
	s.Require().NoError(err)
	s.Require().NoError(ctx.Codec.UnmarshalJSON(bz.Bytes(), respType), bz.String())
	txResp = respType.(*sdk.TxResponse)
	s.Require().Equal(expectedCode, txResp.Code)
	...
	respType = proto.Message(&htlctypes.HTLC{})
	bz, err = QueryHTLCExec(ctx, expectedhtlt.Id)
	s.Require().NoError(err)
	s.Require().NoError(ctx.Codec.UnmarshalJSON(bz.Bytes(), respType))
	...
	bz, err = ClaimHTLCExec(ctx, testCases[1].args.sender.String(), expectedhtlt.Id, testCases[1].args.secret.String(), args...)
	s.Require().NoError(err)
	s.Require().NoError(ctx.Codec.UnmarshalJSON(bz.Bytes(), respType), bz.String())
	txResp = respType.(*sdk.TxResponse)
	s.Require().Equal(expectedCode, txResp.Code)
	...
	bz, err = CreateHTLCExec(ctx, testCases[2].args.sender.String(), args...)
	s.Require().NoError(err)
	s.Require().NoError(ctx.Codec.UnmarshalJSON(bz.Bytes(), respType), bz.String())
	txResp = respType.(*sdk.TxResponse)
	s.Require().Equal(expectedCode, txResp.Code)
	...
	bz, err = ClaimHTLCExec(ctx, testCases[2].args.sender.String(), expectedhtlt.Id, testCases[2].args.secret.String(), args...)
	s.Require().NoError(err)
	s.Require().NoError(ctx.Codec.UnmarshalJSON(bz.Bytes(), respType), bz.String())
	txResp = respType.(*sdk.TxResponse)
	s.Require().Equal(expectedCode, txResp.Code)
