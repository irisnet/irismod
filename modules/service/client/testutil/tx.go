...
func (s *IntegrationTestSuite) TestService() {
	...
	bz, err = QueryServiceDefinitionExec(val.ClientCtx, serviceName)
	s.Require().NoError(err)
	...
	bz, err = QueryServiceBindingExec(val.ClientCtx, serviceName, provider.String())
	s.Require().NoError(err)
	s.Require().NoError(val.ClientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType))
	...
	bz, err = QueryServiceBindingsExec(val.ClientCtx, serviceName)
	s.Require().NoError(err)
	s.Require().NoError(val.ClientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType))
	...
	bz, err = QueryServiceRequestsExec(val.ClientCtx, serviceName, provider.String())
	s.Require().NoError(err)
	s.Require().NoError(val.ClientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType))
	...
	//------test GetCmdQueryServiceRequests()-------------
	respType = proto.Message(&servicetypes.QueryRequestsResponse{})
	bz, err = QueryServiceRequestsByReqCtx(val.ClientCtx, requests[0].RequestContextId, fmt.Sprint(requests[0].RequestContextBatchCounter))
	s.Require().NoError(err)
	s.Require().NoError(val.ClientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType))
	requests = respType.(*servicetypes.QueryRequestsResponse).Requests
	s.Require().Len(requests, 1)
	...
	bz, err = QueryEarnedFeesExec(val.ClientCtx, provider.String())
	s.Require().NoError(err)
	s.Require().NoError(val.ClientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType))
	...
	respType = proto.Message(&sdk.TxResponse{})
	expectedCode = uint32(0)
	bz, err = SetWithdrawAddrExec(clientCtx, withdrawalAddress.String(), provider.String(), args...)
	s.Require().NoError(err)
	s.Require().NoError(clientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType), bz.String())
	txResp = respType.(*sdk.TxResponse)
	s.Require().Equal(expectedCode, txResp.Code)
	...
	respType = proto.Message(&sdk.TxResponse{})
	expectedCode = uint32(0)
	bz, err = WithdrawEarnedFeesExec(clientCtx, provider.String(), provider.String(), args...)
	s.Require().NoError(err)
	s.Require().NoError(clientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType), bz.String())
	txResp = respType.(*sdk.TxResponse)
	s.Require().Equal(expectedCode, txResp.Code)
	...
	bz, err = QueryRequestContextExec(val.ClientCtx, contextId)
	s.Require().NoError(err)
	s.Require().NoError(val.ClientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType))
	...
	bz, err = QueryServiceRequestExec(val.ClientCtx, requestId)
	s.Require().NoError(err)
	s.Require().NoError(val.ClientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType))
	...
	bz, err = QueryServiceResponseExec(val.ClientCtx, requestId)
	s.Require().NoError(err)
	s.Require().NoError(val.ClientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType))
