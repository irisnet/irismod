...
func (s *IntegrationTestSuite) TestNft() {
	...
	bz, err = MintMTExec(val.ClientCtx, from.String(), denomID, mtID, args...)
	s.Require().NoError(err)
	s.Require().NoError(val.ClientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType), bz.String())
	txResp = respType.(*sdk.TxResponse)
	s.Require().Equal(expectedCode, txResp.Code)
	...
	bz, err = QueryMTExec(val.ClientCtx, denomID, mtID)
	s.Require().NoError(err)
	s.Require().NoError(val.ClientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType))
	...
	bz, err = EditMTExec(val.ClientCtx, from.String(), denomID, mtID, args...)
	s.Require().NoError(err)
	s.Require().NoError(val.ClientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType), bz.String())
	txResp = respType.(*sdk.TxResponse)
	s.Require().Equal(expectedCode, txResp.Code)
	...
	bz, err = TransferMTExec(val.ClientCtx, from.String(), recipient.String(), denomID, mtID, args...)
	s.Require().NoError(err)
	s.Require().NoError(val.ClientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType), bz.String())
	txResp = respType.(*sdk.TxResponse)
	s.Require().Equal(expectedCode, txResp.Code)
	...
	bz, err = MintMTExec(val.ClientCtx, from.String(), denomID, newMTID, args...)
	s.Require().NoError(err)
	s.Require().NoError(val.ClientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType), bz.String())
	txResp = respType.(*sdk.TxResponse)
	s.Require().Equal(expectedCode, txResp.Code)
	...
	// }
	respType = proto.Message(&sdk.TxResponse{})
	bz, err = BurnMTExec(val.ClientCtx, from.String(), denomID, newMTID, args...)
	s.Require().NoError(err)
