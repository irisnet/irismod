...
func (suite *KeeperSuite) TestSetCollection() {
	mt := types.NewBaseMT(tokenID, tokenNm, address, tokenURI, tokenURIHash, tokenData)
	// create a new MT and add it to the collection created with the MT mint
	mt2 := types.NewBaseMT(tokenID2, tokenNm, address, tokenURI, tokenURIHash2, tokenData)
