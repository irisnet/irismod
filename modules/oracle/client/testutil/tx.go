...
func (s *IntegrationTestSuite) TestOracle() {
	...
	bz, err = QueryFeedExec(clientCtx, feedName)
	s.Require().NoError(err)
	...
	bz, err = QueryFeedsExec(clientCtx)
	s.Require().NoError(err)
	s.Require().NoError(clientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType))
	...
	bz, err = StartFeedExec(clientCtx, creator.String(), feedName, args...)
	s.Require().NoError(err)
	s.Require().NoError(clientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType), bz.String())
	txResp = respType.(*sdk.TxResponse)
	s.Require().Equal(expectedCode, txResp.Code)
	...
	bz, err = PauseFeedExec(clientCtx, creator.String(), feedName, args...)
	s.Require().NoError(err)
	s.Require().NoError(clientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType), bz.String())
	txResp = respType.(*sdk.TxResponse)
	s.Require().Equal(expectedCode, txResp.Code)
	...
	bz, err = EditFeedExec(clientCtx, creator.String(), feedName, args...)
	s.Require().NoError(err)
	s.Require().NoError(clientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType), bz.String())
	txResp = respType.(*sdk.TxResponse)
	s.Require().Equal(expectedCode, txResp.Code)
	...
	bz, err = QueryFeedValueExec(clientCtx, feedName)
	s.Require().NoError(err)
	s.Require().NoError(clientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType))
