...
func (s *IntegrationTestSuite) TestToken() {
	...
	bz, err := IssueTokenExec(clientCtx, from.String(), args...)
	...
	bz, err = QueryTokensExec(clientCtx, from.String())
	s.Require().NoError(err)
	...
	bz, err = QueryTokenExec(clientCtx, tokenSymbol)
	s.Require().NoError(err)
	...
	bz, err = QueryFeeExec(clientCtx, symbol)
	s.Require().NoError(err)
	s.Require().NoError(clientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType))
	...
	bz, err = QueryParamsExec(clientCtx)
	s.Require().NoError(err)
	s.Require().NoError(clientCtx.Codec.UnmarshalJSON(bz.Bytes(), respType))
	...
	respType = proto.Message(&sdk.TxResponse{})
	bz, err = MintTokenExec(clientCtx, from.String(), symbol, args...)
	...
	respType = proto.Message(&sdk.TxResponse{})
	bz, err = BurnTokenExec(clientCtx, from.String(), symbol, args...)
	...
	respType = proto.Message(&sdk.TxResponse{})
	bz, err = EditTokenExec(clientCtx, from.String(), symbol, args...)
	...
	respType = proto.Message(&sdk.TxResponse{})
	bz, err = TransferTokenOwnerExec(clientCtx, from.String(), symbol, args...)
