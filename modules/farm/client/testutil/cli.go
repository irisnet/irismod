...
func (s *IntegrationTestSuite) TestFarm() {
	...
	bz, err = QueryFarmPoolExec(val.ClientCtx, poolId)
	s.Require().NoError(err)
	...
	bz, err = QueryFarmerExec(val.ClientCtx, creator.String(), queryFarmerArgs...)
	s.Require().NoError(err)
